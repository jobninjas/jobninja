...
import {
  ...
  ClipboardList, Menu, MapPin, Plus, GraduationCap, Code, MessageSquare, Key,
  Gift, Share2
} from 'lucide-react';
          ...
          if (data.profile) {
            ...
            setProfile(prev => {
              ...
              if (data.profile.person || data.profile.address) {
                // Ensure arrays are preserved or initialized if missing from DB
                const merged = { ...prev, ...data.profile };
                merged.employment_history = data.profile.employment_history || [];
                merged.education = data.profile.education || [];
                merged.projects = data.profile.projects || [];
                merged.references = data.profile.references || [];
                return merged;
              }
              ...
              if (data.profile.preferredLocations) newProfile.preferences.preferred_locations = data.profile.preferredLocations;
              if (data.profile.remotePreference) newProfile.preferences.work_mode = data.profile.remotePreference;
              if (data.profile.noticePeriod) newProfile.preferences.notice_period = data.profile.noticePeriod;
              if (data.profile.preferredJobTypes) newProfile.preferences.preferred_job_types = data.profile.preferredJobTypes;

              // Work Auth mapping
              if (data.profile.visaStatus) newProfile.work_authorization.visa_type = data.profile.visaStatus;
              if (data.profile.requiresSponsorship) newProfile.work_authorization.requires_sponsorship_now = data.profile.requiresSponsorship;
              if (data.profile.willingToRelocate) newProfile.work_authorization.relocation_ok = data.profile.willingToRelocate;
